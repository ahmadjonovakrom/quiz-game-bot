...
from database.players import (
    increment_duel_games_played,
    increment_duel_games_won,
)
...
def format_duel_results(results):
    ...
    if winner:
        text += f"🏆 {make_clickable_name(winner)} wins the duel!"
    else:
        text += "🤝 It's a draw!"
...
async def end_game(chat_id, context):
    ...
    async with lock:
        ...
        for row in normalized_results:
            ...
            if mode == "duel":
                increment_duel_games_played(user_id)

        if winner_user_id:
            increment_games_won(winner_user_id)
            if mode == "duel":
                increment_duel_games_won(winner_user_id)
